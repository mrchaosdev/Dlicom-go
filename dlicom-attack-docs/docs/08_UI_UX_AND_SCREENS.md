...
The Loadout area also includes a Skins tab with four Dili costume previews. Night Operative, Solar Vanguard and Glitch Phantom must read as different outfits through silhouette, armor, clothing, weapons and effects rather than palette swaps. Show the selected state clearly, preserve the choice after reload, and explain that changes to an active run's appearance apply next run. The four tabs must fit a 320 CSS-pixel viewport without horizontal scrolling.
