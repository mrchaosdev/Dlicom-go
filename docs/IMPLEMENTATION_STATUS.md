...
Combat hits now include a reusable expanding impact spark; critical hits use a warm flash and a short 3–4 px camera shake, while ultimates use a stronger shake. Reduced-motion settings disable camera shake, and the impact effect reuses one Phaser object instead of creating a display object for every hit. Synthesized dodge, heal, shield, and enemy-death sounds now play from their resolved combat events.
