...
- Core Network normal, elite, and Null.exe ATK/HP values were tuned after a deterministic 500-run max-level-gear simulation showed only 82 clears and 96 deaths before the boss. Current values preserve the 18% NULL PULSE and lower the base attack pressure / boss health: the same fixed-seed bot now reaches Null.exe in 484/500 runs and wins 180/500. This is a reversible balance default, not a human-playtest result; the heuristic and selected build are documented in `scripts/simulate.ts`.
