...
- `public/assets/attack.wav`, `crit.wav`, `ultimate.wav`, `select.wav` and four chapter loops (`feed-loop.wav`, `dliclips-loop.wav`, `rooms-loop.wav`, `core-loop.wav`): original synthesized audio from `scripts/audio.mjs`. Run `node scripts/audio.mjs` to reproduce. No samples or third-party music used.
